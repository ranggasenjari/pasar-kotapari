...
class Cari_model extends CI_Model {
    ...
    function num_selecttoko($q,$key,$val)
	{
		$keyword = mysqli_real_escape_string(get_instance()->db->conn_id,$q);
		$this->db->select("dt_usaha.*, districts.name as kec, villages.name as des, usr_users.id_kecamatan as idkec_user, usr_users.id_desa as iddesa_user, usr_users.*");
		$this->db->from('dt_usaha');
		$this->db->join('districts', 'dt_usaha.id_kecamatan = districts.id', 'left');
		$this->db->join('villages', 'dt_usaha.id_desa = villages.id', 'left');
		$this->db->join('usr_users', 'dt_usaha.id_pemilik = usr_users.user_id', 'left');
		$this->db->where("MATCH(nama_usaha,detil) AGAINST('".$keyword."')");
		$this->db->where($key,$val);
		return $this->db->get()->num_rows();
	}
	
    function cari_produk($key,$filter,$offset,$sort)
	{
		$hasil ="";
		$auto =0;
		$limit =30;
       ...
       $hasil .='<div class="box-header ">
          <h3 class="box-title">Hasil pencarian untuk "'.$key.'" ('.$t.' hasil)</h3>
        </div>
		<div class="box-body">
			<div class="row">';
    ...
    function get_kec_tokofilter($key)
	{
		$hasil ="";
		$hasil .='<div class="form-group">
                <label>Kecamatan</label>
		...
		$i=1;
		foreach ($q->result() as $d){
			$num[$i] = $this->num_selecttoko($key,'dt_usaha.id_kecamatan',$d->id);
			$hasil .='<option value="'.$d->id.'">'.$d->name.' ('.$num[$i].')</option>';
			$i++;
		}
		$hasil .='</select>
    ...
    function get_kat_tokofilter($key)
	{
		$hasil ="";
		$hasil .='<div class="form-group">
		...
		$i=1;
		foreach ($q->result() as $d){
			$num[$i] = $this->num_selecttoko($key,'id_kat_usaha',$d->id_kat_usaha);
			$hasil .='<option value="'.$d->id_kat_usaha.'">'.$d->uraian.' ('.$num[$i].')</option>';
			$i++;
		}
		$hasil .='</select>
				</div>';
    ...
    // function get_katproduk()
	// {
		// $hasil ="";
		// $hasil .='<div class="form-group">
                // <label>Kategori Produk</label>
                // <select id="katproduk" name="katproduk" class="form-control input-sm" data-placeholder="Semua Kategori" style="width: 100%;">';
		// $hasil .='<option value="">- Pilih Kategori -</option>';
		// $q = $this->db->get('dt_kat_produk');
		// foreach ($q->result() as $d){
			// $hasil .='<option value="'.$d->id_kat_produk.'">'.$d->uraian.'</option>';
		// }
		// $hasil .='</select>
				// </div>';
